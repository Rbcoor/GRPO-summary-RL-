...
- `SAMPLES_PER_DOCUMENT`：每篇文档独立生成并评测的 workflow 轨迹数，默认 `1`
- `SUMMARY_TEMPERATURE`：摘要侧采样温度，默认 `0.0`。生成 GRPO 用 K 轨迹时建议使用非零值，例如 `0.7`
- `SUMMARY_TOP_P`：摘要侧 top-p，默认 `1.0`
...
生成适合 GRPO 分组的离线候选轨迹时，需要让同一篇文档产生多条独立 workflow，并分别 judge：
...
输出目录会使用 `<validation_index>_s<sample_index>_<document_id>` 命名，例如
`000_s00_tabybkba/` 和 `000_s01_tabybkba/`。`validation_items.jsonl`、
`validation_report.json` 和每个 `<document_id>.trajectory.json` 的 metadata
都会记录 `sample_index` 与 `run_index`，后续训练可以按 `document_id` 或
`validation_index` 将同一文档的 K 条候选轨迹组成一个 `TrajectoryGroup`。
...
该脚本默认按原始在线 GRPO 的设计组织训练样本：一个文档对应一个 `TrajectoryGroup`，组内应包含同一文档的多条独立采样轨迹。若所有 document group 都只有单一 reward，脚本会在加载模型前报错，避免出现“流程跑完但 loss/grad 为 0”的假训练。只有在单纯验证链路时，才应显式传入 `--allow-zero-variance-groups`。

对于一次 workflow 内部的多个 `model_call`，脚本可以通过重复传 `--step` 选择训练动作，例如 `--step initial_summary --step revision_summary`。这会把选中的摘要动作转换为 ART trajectory，但它们不能替代同一文档的 K 次独立 rollout；真正的 GRPO 数据仍应通过 `SAMPLES_PER_DOCUMENT=K` 生成。
...
- `--trajectory-jsonl`：合并后的 JSONL 轨迹索引文件。若传入该参数，脚本会按其中的 `trajectory_path` 读取最终轨迹。
- `--limit`：使用前多少篇轨迹，默认 `10`。
- `--step`：训练的模型调用步骤，可重复传入。默认 `initial_summary`。
- `--grouping`：ART group 组织方式，默认 `document`，即按文档成组。`single` 仅用于调试。
- `--allow-zero-variance-groups`：允许零 reward 方差 group 继续训练，只建议用于 smoke test。
